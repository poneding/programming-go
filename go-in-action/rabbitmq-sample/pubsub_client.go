...
func NewPubSubClient(url, exchange string) *PubSubClient {
	return &PubSubClient{
		// 订阅消费模式下
		// 1. qname 为空
		// 2. key 为空
		newClient(url, "", exchange, ""),
	}
}

func (mq *PubSubClient) Publish(msg string) {
	// 获取交换机
...
func (mq *PubSubClient) Consume(consumeFunc func(msg string)) {
	// 获取交换机
	err := mq.channel.ExchangeDeclare(
		mq.exchange,
		"fanout", // fanout，广播
