...
func NewTopicClient(url, exchange, topicKey string) *TopicClient {
	return &TopicClient{
		newClient(url, "", exchange, topicKey),
	}
}
func (mq *TopicClient) Publish(msg string) {
	// 获取交换机
...
func (mq *TopicClient) Consume(consumeFunc func(msg string)) {
	// 获取交换机
	err := mq.channel.ExchangeDeclare(
		mq.exchange,
		"topic", // direct, 单向
